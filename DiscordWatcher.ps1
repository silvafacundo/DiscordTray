...
try {
	...
	# Register the event
	$handler = Register-ObjectEvent $Watcher -EventName "Created" -SourceIdentifier "DiscordDetection" -Action {
	...
	Write-Host "Listening for Discord.exe creation events..."
	do {
		Wait-Event -Timeout 1
	} while ($true)
} finally {
	Write-Host "Stopping Discord Detection..."
	# Unregister the event
	Unregister-Event -SourceIdentifier "DiscordDetection"
	...
	Write-Host "Discord Detection Stopped..."
}
